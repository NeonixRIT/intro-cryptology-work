...
def permute(inp: bytes | int, permutation: tuple[int], base: int = 1, return_bin=False, zfill_len=None) -> int | str:
    ...
    if zfill_len is None:
        zfill_len = max(permutation)
    inp = bin(inp)[2:].zfill(zfill_len)
    res = ''.join([inp[i - base] for i in permutation])
...
def rotr(items: str | list, shift: int) -> str | list:
    shift %= len(items)
    if shift == 0:
        return items
...
def rotl(items: str | list, shift: int) -> str | list:
    shift %= len(items)
    if shift == 0:
        return items
    return items[shift:] + items[:shift]
...
# DES supports ECB, CBC, CFB, OFB modes
# 64 bit init key, feistel network
